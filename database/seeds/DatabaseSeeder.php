    ...
    public function run()
    {
		...
		for($i_prov = 0; $i_prov < 1; $i_prov++){
			...
			for($i_kab = 0; $i_kab < 1; $i_kab++){
				...
				$kab->save();
				for($i_kec = 0; $i_kec < rand(5, 10); $i_kec++){
					$user = new App\User;
					$user->name = $faker->firstName.' '.$faker->lastName;
					$user->email = strtolower(str_replace(' ', '', $user->name)).'@komoditas.id';
					$user->phone = $user->email;
					...
					$kec->save();
					for($i_des = 0; $i_des < rand(5, 10); $i_des++){
						$user = new App\User;
						$user->name = $faker->firstName.' '.$faker->lastName;
						$user->email = strtolower(str_replace(' ', '', $user->name)).'@komoditas.id';
						$user->phone = $user->email;
						...
						for($i_lahan = 0; $i_lahan < rand(5, 10); $i_lahan++){
							$lahan = new App\Lahan;
							...
							if($komoditas['type'] == 'ternak'){
								...
								$lk->b_tanggal_panen = $faker->dateTimeBetween($startDate = 'now', $endDate = '+3 months', $timezone = null);
							...
							if($komoditas['type'] == 'tanaman'){
								...
								$lk->t_tanggal_panen = $faker->dateTimeBetween($startDate = 'now', $endDate = '+3 months', $timezone = null);
							...
							if($komoditas['type'] == 'ikan'){
								...
								$lk->i_tanggal_panen = $faker->dateTimeBetween($startDate = 'now', $endDate = '+3 months', $timezone = null);
